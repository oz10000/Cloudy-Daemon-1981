...
class PositionManager:
        ...
        for pos in positions:
            ...
            if pos.trailing_distance > 0:
                if pos.direction == 'LONG':
                    if pos.highest_price >= pos.trailing_activation:
                        ...
                        if new_sl > pos.sl:
                            pos.sl = new_sl
                            self.logger.debug("TRAILING", f"SL actualizado a {new_sl:.2f} para {pos.id}")
                else:
            ...
            if pos.breakeven_activation > 0:
                ...
                if profit_percent >= pos.breakeven_activation:
                    # Mover SL a entry (breakeven)
                    if pos.sl != pos.entry_price:
                        pos.sl = pos.entry_price
                        self.logger.debug("BREAKEVEN", f"SL movido a breakeven para {pos.id}")
    ...
    def set_trailing_stop(self, position_id: str, activation: float, distance: float) -> bool:
        ...
        self.logger.info("POSITION", f"Trailing activado para {position_id} (activation={activation}, dist={distance})")
        return True
    ...
    def set_breakeven(self, position_id: str, activation: float) -> bool:
        ...
        self.logger.info("POSITION", f"Breakeven activado para {position_id} (activation={activation})")
        return True
